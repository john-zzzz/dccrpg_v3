...
export const spells = {
	ekimMysticalMask: {
		...
		range: 'Self',
		duration: { timeUnit: timeUnits.round, number: { _ref: 'level.key' } },
		castingTime: { timeUnit: timeUnits.action, number: 1 },
		castingDie: { _ref: 'spellCasting.die', _default: { die: dice.d20, number: 1, name: 'Cast' } },
		general:
		...
		manifestation: {
			dice: { die: dice.d4, number: 1, name: 'Manifestation' },
			notes:
		...
		corruption: {
			dice: { die: dice.d4, number: 1, name: 'Corruption' },
			results: [
				...
				{
					roll: 3,
					result: 'The caster develops a phobia about revealing his true face and takes to wearing veils or hooded cloaks.'
				},
				{ roll: 4, result: "The caster's nose vanishes completely, leaving his face flat and mask-like." }
			]
		},
		misfire: {
			dice: { die: dice.d4, number: 1, name: 'Misfire' },
			results: [
				{
					roll: 1,
					result:
		...
		results: [
			{
				roll: 1,
				result: 'Lost, failure, and worse! Roll 1d6 modified by Luck: (0 or less) corruption + patron taint + misfire; (1-2) corruption; (3+) misfire.',
				failure: true,
				failureDice: {
					dice: { die: dice.d6, number: 1, modifier: { _ref: 'luck.currentModifier', _max: 0 }, name: 'Failure' },
					results: [
                        // TODO: Figure out a way to handle results < 0 and greater than max.
						{
							roll: -3,
							corruption: true,
							patronTaint: true,
	...
	spiderClimb: {
		...
		range: 'Self or touch',
		duration: { timeUnit: timeUnits.round, value: { token: 'level' } },
		castingTime: { timeUnit: timeUnits.action, value: 1 },
		...
		corruption: {
			...
			results: [
				{ roll: 1, result: 'Caster grows four large spider-like limbs from his back.' },
				{
					roll: 2,
					result:
						"Caster can spin small webs like a spider and throw them up to 30' as a sticky goo (ranged attack roll, DC 12 Strength or Agility check for target to escape)."
				},
		...
		misfire: {
			...
			results: [
				{
					roll: 1,
					result: 'Caster sticks himself to the floor and cannot move his feet until he makes a DC 16 Strength check.'
				},
				{
					roll: 2,
					result:
						'Caster makes his appendages magically slippery and has trouble standing straight for the next 1d6 rounds, falling over constantly unless he makes a DC 12 Agility check each round.'
				},
				{
					roll: 3,
					result:
		...
		results: [
			{
				roll: 1,
				result: 'Lost, failure, and worse! Roll 1d6 modified by Luck: (0 or less) corruption + misfire; (1-2) corruption; (3+) misfire.',
				failure: true,
				dice: {
					die: dice.d6,
					number: 1,
